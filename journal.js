...
const state = {
  ...
  unsubTrades: null,
  editingTradeId: null,
  editingTradeRisk: null, // risk (USD) captured when the trade was added; Edit form has no Risk input of its own
...
function fmtMoney(n) {
  // Missed trades intentionally store profitLoss as null (not 0) — show
...
/* -----------------------------------------------------------------
   ...
   ----------------------------------------------------------------- */
/** Returns a toggle button's underlying value: its data-value attribute
 *  when present (used by the Result toggle so the visible label can say
 *  "Break-Even" while the stored value stays the backward-compatible
 *  "BE"), otherwise falls back to the button's text. */
...
function wireToggleGroup(container) {
  ...
  container.addEventListener("click", (e) => {
    ...
    const label = toggleBtnValue(btn);
    if (label === "Win") btn.classList.add("jr-toggle-win");
    if (label === "Loss") btn.classList.add("jr-toggle-loss");
    // Result toggles (Win/Loss/BE/Partial/Missed) drive conditional
    // visibility of execution-only fields when "Missed" is selected.
    if (container.id === "addResultToggle") applyMissedVisibility("add");
    if (container.id === "editResultToggle") applyMissedVisibility("edit");
  });
}
function getToggleValue(container) {
  const sel = container && container.querySelector(".jr-toggle-btn.jr-selected");
  return sel ? toggleBtnValue(sel) : "";
}
function setToggleValue(container, value) {
  if (!container) return;
  $all(".jr-toggle-btn", container).forEach(b => {
    const match = toggleBtnValue(b) === value;
...
/** Result = "Missed" means "I saw a valid setup but didn't take it" —
 *  it's an opportunity record, not an executed trade. This hides (and
 *  therefore stops requiring) the execution-only fields — Entry, Exit,
 *  Lot Size, SL/TP, P/L, Risk, R:R, Close Reason — for both the Add and
 *  Edit forms, and shows a short explanatory hint instead. Nothing is
 *  removed from the DOM; values are simply not collected/validated
 *  while hidden (see collectTradeFormData / validateTradeForm). */
function applyMissedVisibility(prefix) {
  const modal = byId(prefix === "add" ? "addTradeModal" : "editTradeModal");
  if (!modal) return;
  const resultToggle = byId(prefix === "add" ? "addResultToggle" : "editResultToggle");
  const isMissed = getToggleValue(resultToggle) === "Missed";
  $all(".jr-hide-missed", modal).forEach(field => { field.style.display = isMissed ? "none" : ""; });
  const hint = byId(prefix === "add" ? "addMissedHint" : "editMissedHint");
  if (hint) hint.style.display = isMissed ? "" : "none";
}
...
function updateStats() {
  ...
  if (countLabel) {
    const missedCount = trades.filter(t => t.result === "Missed").length;
    countLabel.textContent = `${trades.length} trade${trades.length === 1 ? "" : "s"} recorded${missedCount ? ` (${missedCount} missed)` : ""}`;
  }
...
function renderRow(t, i) {
  ...
  const plClass = (t.profitLoss === null || t.profitLoss === undefined) ? "" : ((Number(t.profitLoss) || 0) >= 0 ? "jr-profit-pos" : "jr-profit-neg");
  return `
...
function validateTradeForm(prefix) {
  ...
  const resultToggle = prefix === "add" ? byId("addResultToggle") : byId("editResultToggle");
  const isMissed = getToggleValue(resultToggle) === "Missed";

  if (!instrument) errors.push("Instrument / Pair is required.");
  if (!dateField) errors.push("Trade date is required.");

  // A "Missed" result is an opportunity record, not an executed trade —
  // entry/exit/lot size/P&L are optional and must NOT be required or
  // defaulted to fake values (0, BE, etc).
  if (!isMissed) {
...
/** Live-updates the read-only R-Multiple field as the user types, using
 *  the same formula as collectTradeFormData (profit / risk). Purely a
 *  display convenience — the authoritative value is recomputed on save. */
function refreshRMultipleDisplay(prefix) {
  const rEl = byId(`${prefix}RMultiple`);
  if (!rEl) return;
  const resultToggle = byId(prefix === "add" ? "addResultToggle" : "editResultToggle");
  const result = getToggleValue(resultToggle) || "Win";
  if (result === "Missed") { rEl.value = ""; return; }
  const plField = prefix === "add" ? "addProfitLoss" : "editNetProfit";
  const riskEl = byId("addRisk"); // Risk (USD) input only exists on the Add form
  const risk = prefix === "add" ? Number(riskEl?.value) : Number(state.editingTradeRisk);
  const plRaw = byId(plField)?.value;
  if (!risk || plRaw === "" || plRaw === undefined) { rEl.value = ""; return; }
  const profitLoss = computeSignedProfitLoss(plRaw, result);
  rEl.value = (profitLoss / risk).toFixed(2);
}
function wireRMultipleAutoCalc(prefix) {
  const ids = prefix === "add" ? ["addProfitLoss", "addRisk"] : ["editNetProfit"];
  ids.forEach(id => { const el = byId(id); if (el) el.addEventListener("input", () => refreshRMultipleDisplay(prefix)); });
}
...
function collectTradeFormData(prefix) {
  ...
  /** Reads a numeric field as a real number, or null when blank/not
   *  applicable — never fabricates a 0 for data that wasn't entered
   *  (matters most for Missed trades, which have no execution data). */
  const numOrNull = (id) => {
  ...
  const data = {
    ...
    tradeDate,
    tradeType: val(`${prefix}TradeType`) || "Live",
    result,
    isMissed,
    lotSize: numOrNull(`${prefix}LotSize`),
    entryPrice: numOrNull(`${prefix}EntryPrice`),
    exitPrice: numOrNull(`${prefix}ExitPrice`),
    stopLoss: numOrNull(`${prefix}StopLoss`),
    takeProfit: numOrNull(`${prefix}TakeProfit`),
    session: val(`${prefix}Session`),
    strategy: val(`${prefix}Strategy`) || null,
    setup: val(`${prefix}Setup`) || null,
    marketCondition: val(`${prefix}MarketCondition`) || null,
    notes: val(`${prefix}Notes`) || val(`${prefix}TradeNotes`) || "",
    tradeRating: getToggleValue(byId(`${prefix}TradeGrade`)) || null,
    mistake: val(`${prefix}MistakeReason`) || "None",
    updatedAt: serverTimestamp()
  };

  // Close Reason exists on both forms. Risk (USD) is only an input on the
  // Add form — the Edit form has no Risk field, so on edit we preserve
  // whatever risk was originally recorded rather than wiping it to null.
  data.closeReason = isMissed ? null : (val(`${prefix}CloseReason`) || "None");
  if (prefix === "add") {
    const rawRisk = isMissed ? "" : val("addRisk");
    data.risk = rawRisk !== "" ? Number(rawRisk) : null;
  } else {
    const original = state.trades.find(t => t.id === state.editingTradeId);
    data.risk = isMissed ? null : (original && original.risk != null ? original.risk : null);
  }

  if (prefix === "add") {
    data.entryTime = val("addEntryTime");
    data.exitTime = isMissed ? "" : val("addExitTime");
    // Missed trades are opportunity records, not executed trades: P/L
    // must stay null (never 0/BE) so Analyze can tell them apart.
    data.profitLoss = isMissed ? null : computeSignedProfitLoss(val("addProfitLoss"), result);
    data.riskReward = isMissed ? "" : (val("addRiskReward") || "");
    ...
    data.commission = isMissed ? null : (val("editCommission") !== "" ? Number(val("editCommission")) : null);
    data.profitLoss = isMissed ? null : computeSignedProfitLoss(val("editNetProfit"), result);
    ...
    data.confidence = val("editConfidence") !== "" ? Number(val("editConfidence")) : null;
    data.discipline = val("editDiscipline") !== "" ? Number(val("editDiscipline")) : null;
    data.followedPlan = getToggleValue(byId("editFollowedPlan")) || "Yes";
    data.lessons = val("editLessons");
  }

  // R-Multiple: only for completed trades, and only when risk is known.
  // Never invented — left null when the inputs to compute it are missing.
  data.rMultiple = (!isMissed && data.risk) ? Number((data.profitLoss / data.risk).toFixed(2)) : null;
...
function resetAddForm() {
  ["addInstrument","addLotSize","addEntryTime","addExitTime","addEntryPrice","addExitPrice","addStopLoss",
   "addTakeProfit","addProfitLoss","addRisk","addRiskReward","addRMultiple","addStrategy","addSetup","addConfidence",
  ...
  const tradeTypeEl = byId("addTradeType"); if (tradeTypeEl) tradeTypeEl.value = "Live";
  const closeReasonEl = byId("addCloseReason"); if (closeReasonEl) closeReasonEl.value = "None";
  const mistakeReasonEl = byId("addMistakeReason"); if (mistakeReasonEl) mistakeReasonEl.value = "None";
...
function openEditModal(trade) {
  ...
  state.editingTradeRisk = trade.risk ?? null;
  ...
  if (byId("editTradeType")) byId("editTradeType").value = trade.tradeType || "Live";
  byId("editLotSize").value = trade.lotSize ?? "";
  byId("editSession").value = trade.session || "Asia";
  if (byId("editMarketCondition")) byId("editMarketCondition").value = trade.marketCondition || "Trending";
  if (byId("editStrategy")) byId("editStrategy").value = trade.strategy || "";
  if (byId("editSetup")) byId("editSetup").value = trade.setup || "";
  ...
  if (byId("editMistakeReason")) byId("editMistakeReason").value = trade.mistake || "None";
  byId("editNotes").value = trade.notes || "";
  if (byId("editConfidence")) byId("editConfidence").value = trade.confidence ?? "";
  if (byId("editDiscipline")) byId("editDiscipline").value = trade.discipline ?? "";
  if (byId("editLessons")) byId("editLessons").value = trade.lessons || "";
  setToggleValue(byId("editResultToggle"), trade.result === "BE" ? "BE" : (trade.result || "Win"));
  setToggleValue(byId("editTradeGrade"), trade.tradeRating || "");
  setToggleValue(byId("editFollowedPlan"), trade.followedPlan || "Yes");
  applyMissedVisibility("edit");
  refreshRMultipleDisplay("edit");
...
/* -----------------------------------------------------------------
   ...
   ----------------------------------------------------------------- */
const CSV_COLUMNS = ["instrument","direction","tradeType","date","entryTime","exitTime","lotSize","entryPrice","exitPrice",
  "stopLoss","takeProfit","profitLoss","risk","riskReward","rMultiple","result","isMissed","closeReason","mistake",
  "strategy","setup","session","marketCondition","timeframe","rating","notes"];
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += batchSize) {
        ...
        chunk.forEach(row => {
          ...
          const resultRaw = idx("result") >= 0 ? row[idx("result")] : "Win";
          const isMissed = (resultRaw || "").trim().toLowerCase() === "missed";
          const entryPrice = idx("entryprice") >= 0 && row[idx("entryprice")] !== "" ? Number(row[idx("entryprice")]) : NaN;
          const exitPrice = idx("exitprice") >= 0 && row[idx("exitprice")] !== "" ? Number(row[idx("exitprice")]) : NaN;
          const lotSize = idx("lotsize") >= 0 && row[idx("lotsize")] !== "" ? Number(row[idx("lotsize")]) : NaN;
          // Missed rows are opportunity records — entry/exit/lot size are
          // optional. Executed-trade rows still require all three.
          if (!instrument || !dateStr) { skipped++; return; }
          if (!isMissed && (isNaN(entryPrice) || isNaN(exitPrice) || isNaN(lotSize))) { skipped++; return; }
          ...
          batch.set(docRef, {
            ...
            tradeType: idx("tradetype") >= 0 && row[idx("tradetype")] ? row[idx("tradetype")] : "Live",
            tradeDate,
            entryTime: idx("entrytime") >= 0 ? row[idx("entrytime")] : "",
            exitTime: isMissed ? "" : (idx("exittime") >= 0 ? row[idx("exittime")] : ""),
            lotSize: isMissed ? null : (isNaN(lotSize) ? null : lotSize),
            entryPrice: isMissed ? null : (isNaN(entryPrice) ? null : entryPrice),
            exitPrice: isMissed ? null : (isNaN(exitPrice) ? null : exitPrice),
            stopLoss: !isMissed && idx("stoploss") >= 0 && row[idx("stoploss")] !== "" ? Number(row[idx("stoploss")]) : null,
            takeProfit: !isMissed && idx("takeprofit") >= 0 && row[idx("takeprofit")] !== "" ? Number(row[idx("takeprofit")]) : null,
            // Missed trades must never get a fabricated P/L — null, not 0.
            profitLoss: isMissed ? null : (idx("profitloss") >= 0 && row[idx("profitloss")] !== "" ? Number(row[idx("profitloss")]) : null),
            risk: !isMissed && idx("risk") >= 0 && row[idx("risk")] !== "" ? Number(row[idx("risk")]) : null,
            riskReward: isMissed ? "" : (idx("riskreward") >= 0 ? row[idx("riskreward")] : ""),
            rMultiple: !isMissed && idx("rmultiple") >= 0 && row[idx("rmultiple")] !== "" ? Number(row[idx("rmultiple")]) : null,
            result: resultRaw || "Win",
            isMissed,
            closeReason: isMissed ? null : (idx("closereason") >= 0 ? row[idx("closereason")] || "None" : "None"),
            mistake: idx("mistake") >= 0 && row[idx("mistake")] ? row[idx("mistake")] : "None",
...
function wireCsvExport() {
  ...
  exportBtn.addEventListener("click", () => {
    ...
    const rows = trades.map(t => {
      const rowMap = {
        instrument: t.instrument, direction: t.direction, tradeType: t.tradeType || "Live",
        date: getTradeDate(t)?.toISOString().slice(0,10) || "",
        ...
        risk: t.risk ?? "", riskReward: t.riskReward || "", rMultiple: t.rMultiple ?? "", result: t.result || "",
        isMissed: t.result === "Missed" ? "true" : "false", closeReason: t.closeReason || "", mistake: t.mistake || "None",
        strategy: t.strategy || "", setup: t.setup || "", session: t.session || "", marketCondition: t.marketCondition || "",
...
async function boot() {
  ...
  wireRMultipleAutoCalc("add");
  wireRMultipleAutoCalc("edit");
  applyMissedVisibility("add");
  applyMissedVisibility("edit");
