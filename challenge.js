...
async function switchAccount(accountId, toast = true) {
  ...
  // Clear the previous account's trades immediately so a switch can never
  // render one account's numbers under another account's name, even for a
  // single frame while the new subscription's first snapshot is in flight.
  state.trades = [];
...
function wireRiskSettings(){
  document.getElementById("riskSettingsForm")?.addEventListener("submit",saveRiskSettingsFromModal);
  document.addEventListener("click",e=>{
    if(e.target.closest && e.target.closest('[data-modal-close="riskSettingsModal"]'))closeRiskSettingsModal();
    if(e.target.id==="riskSettingsModalOverlay")closeRiskSettingsModal();
  });
}
 ...
 * BOOTSTRAP
 * ---------------------------------------------------------------
 * Wires every control exactly once, then hands account/trade loading
 * to Firebase Auth's state listener — the single source of truth for
 * "who is signed in" and therefore for which Firestore paths
 * (users/{uid}/accounts/...) are valid to read. Nothing above this
 * point performs any Firestore read on its own; without this block
 * the page has no data source and no event handlers are attached.
 * (This block, along with wireRiskSettings() above, was missing from
 * the file entirely — it ended mid-declaration at "function wire".)
 */
function wireAll(){
  wireAddTrade();
  wireCreateAccount();
...
function clearAccountState(){
  ...
  state.optional = {};
  state.lastMetrics = null;
  const el = root(); if (el) el.dataset.accountId = "";
}
...
onAuthStateChanged(auth, user => {
  if (user) {
    state.uid = user.uid;
    state.user = user;
    loadAccounts().catch(err => showToast(friendlyError(err), "error"));
  } else {
    state.uid = null;
    state.user = null;
    clearAccountState();
