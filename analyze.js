...
function normalizeTrade(raw) {
  ...
  // A Missed trade is an opportunity that was NOT executed — it must never
  // be treated as a $0 (Break-Even) trade. Checked first so a missing/null
  // profitLoss on a Missed trade can never fall through to fabricated data.
  const isMissed = raw.isMissed === true || storedResult === "MISSED";

  // Never coerce a missing/null/blank P/L into 0 — that would fabricate an
  // executed Break-Even result out of a trade that simply has no data yet.
  const hasPL = raw.profitLoss !== undefined && raw.profitLoss !== null && raw.profitLoss !== "";
  const plNum = hasPL ? Number(raw.profitLoss) : null;
  const profitLoss = (plNum !== null && !isNaN(plNum)) ? plNum : null;

  let classification;
  if (isMissed) {
    classification = "MISSED"; // never WIN/LOSS/BREAK-EVEN, never counted in P/L or win-rate math
  } else if (profitLoss !== null) {
    classification = profitLoss > 0 ? "WIN" : profitLoss < 0 ? "LOSS" : "BREAK-EVEN";
  } else {
    classification = null; // executed trade with genuinely missing P/L — insufficient data, not fabricated
  }
  const inconsistent = !isMissed && storedResult && classification && storedResult !== classification &&
  ...
  // Prefer the rMultiple Journal already computed and stored — never
  // recompute/fabricate a different R than the one the user saved. Only
  // derive one locally when the stored value is absent and the trade
  // genuinely has what's needed to calculate it (never for Missed trades).
  let rMultiple = null;
  const hasStoredR = raw.rMultiple !== undefined && raw.rMultiple !== null && raw.rMultiple !== "" && !isNaN(Number(raw.rMultiple));
  if (hasStoredR) {
    rMultiple = Number(raw.rMultiple);
  } else if (!isMissed && risk && risk !== 0 && profitLoss !== null) {
    rMultiple = profitLoss / Math.abs(risk);
  }
  ...
  return {
    ...
    id: raw.id,
    isMissed,
    plSigned: profitLoss === null ? 0 : profitLoss, // safe additive default; classification (not this) gates all win/loss/PL math
    hasPL: profitLoss !== null,
    classification,          // "WIN" | "LOSS" | "BREAK-EVEN" | "MISSED" | null — always trust this, never raw `result`
...
function showNoDataState() {
  ...
  ["stat-total-trades","stat-net-pl","stat-win-rate","stat-avg-win","stat-avg-loss","stat-profit-factor",
    ...
    "stat-winning-trades","stat-losing-trades","stat-breakeven-trades","stat-missed-trades"].forEach(id => setText(id, "—"));
...
function recomputeAndRender() {
  ...
  // Missed trades are logged opportunities, not executed trades — split out
  // here, once, so every calculation below (P/L, win rate, profit factor,
  // expectancy, R, equity, drawdown, and every category breakdown) only
  // ever sees genuinely executed trades. A trade with no result yet
  // (classification === null) is likewise excluded rather than guessed at.
  const missedTrades = allTrades.filter(t => t.classification === "MISSED");
  const trades = allTrades.filter(t => t.classification !== "MISSED" && t.classification !== null);
  ...
  setText("stat-missed-trades", String(missedTrades.length));

  const summarySub = byId("an-filter-summary");
  const missedNote = missedTrades.length ? ` (+${missedTrades.length} missed opportunit${missedTrades.length === 1 ? "y" : "ies"})` : "";
  if (summarySub) summarySub.textContent = `${trades.length} trade${trades.length === 1 ? "" : "s"} match current filters${missedNote}`;
}
...
function sessionBucket(trade) {
  ...
  if (s === "new york am") return "ny-am";
  if (s === "new york pm") return "ny-pm";
  return null;
}
function renderSessionSection(trades) {
  ...
  trades.forEach(t => {
    ...
    else if (s === "new york am") buckets["ny-am"].push(t);
    else if (s === "new york pm") buckets["ny-pm"].push(t);
    // Other real session values (New York, New York Pre-Market, New York
    // Open, London/New York Overlap, or any custom value) have no fixed
    // card — they still appear below in the generic per-session list.
  });
  ["asian", "london", "ny-am", "ny-pm"].forEach(key => fillSessionCard(key, buckets[key]));

  // Every distinct session value present in the data, not just the four
  // fixed cards above — so New York, New York Pre-Market, New York Open,
  // and London/New York Overlap are never silently dropped.
  const groups = statsForGroups(groupBy(trades, t => t.session)).filter(g => g.stats.total > 0);
